...
class TraceExportEngine:
    ...
    def generate_export(db: Session, export_job: ExportJob, filters: CohortFilter) -> str:
        ...
        # Fetch domain events
        conds = db.query(TraceCondition).filter(TraceCondition.encounter_key.in_(enc_keys)).all()
        procs = db.query(TraceProcedure).filter(TraceProcedure.encounter_key.in_(enc_keys)).all()
